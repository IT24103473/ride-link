...
/**
 * Security for the Fare &amp; Payment Service.
 *
 * <p>Verification only - this service holds no signing key and issues no tokens. It is a
 * near-copy of the other services' configuration minus the encoder.</p>
 *
 * <p>There are deliberately no {@code /api/v1/internal/**} endpoints here: nothing in this
 * service is called service-to-service. Fares and payments are driven either by a
 * passenger's own request or by a ride event off the broker, so every HTTP endpoint
 * belongs to a person. The rules that matter are therefore ownership rules - "was I on
 * this ride?" - which live in the service layer where the data is visible, rather than in
